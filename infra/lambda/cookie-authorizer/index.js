...
function mainProcess(event, context, callback) {
  ...
  if (request.uri.startsWith(config.CALLBACK_PATH)) {
    ...
    axios
      .post(config.TOKEN_ENDPOINT, postData)
      .then(function (response) {
        ...
        if (responseQueryString.error) {
          ...
          axios
            .get('https://api.github.com/user', { headers: { Authorization: authorization } })
            .then(function (response) {
              console.log(response);
              ...
              axios
                .get(orgsGet, { headers: { Authorization: authorization } })
                .then(function (response) {
                  console.log(response);
                  ...
                  if (response.status == 204) {
                    ...
                    unauthorized(
                      'Unauthorized. User ' + response.login + ' is not a member of required organization.',
                      callback,
                    );
                  }
                })
