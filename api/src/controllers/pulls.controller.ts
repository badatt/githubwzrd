...
export const relatedPull = async (req: Request, res: Response, next: NextFunction) => {
  const { repo } = req.params;
  const { org, gitToken, userId, username } = req.currentUser;
  const userItem = await db.tryGetAsync(new User(), { id: userId, org: org });
  if (!userItem.repos || userItem.repos.length === 0)
    throw new APIError({ message: 'No repos found', status: NOT_FOUND });
  if (!userItem.repos.includes(repo))
    throw new APIError({ message: 'Do not have access to the repo', status: UNAUTHORIZED });
  ...
  res.send({ data: relatedPull, rateLimit });
};
